...
import re
from minerva.util.general_utils import levenshtein
...
def fixNumberCitations(text):
    ...
    while len(old_text) != len(text):
        old_text=text
        text=re.sub(r"(\[(?:\d+(?:-|,))*)(\d+)(?:-|--|&#x02013;)(\d+)((?:,\d+)*\])", repFunc, text, flags=re.IGNORECASE)

    old_text=""
    while len(old_text) != len(text):
        old_text=text
        text=re.sub(r"(?:(?:\[([\d\-\s,]+)\][ ,]+\[([\d\-,])+\]))", r"[\1,\2]", text, flags=re.IGNORECASE)

    return unicode(text)


def detectCitationStyle(text, citation_regexes=CITATION_REGEXES):
    """
...
def matchCitationWithReference(citation_data, references):
    ...
    if citation_data["etal"]:
        num_authors=(3,100) # at least 3 authors for et al.
    else:
        if citation_data["a2"]:
...
def selectMatchableCitations(doc):
    """
...
##    print ("total citations: %s" % len(doc["citations"]))
    for cit in doc["citations"]:
        if cit["ref_id"] != 0:
            ref=doc.matchReferenceById(cit["ref_id"])
            if ref:
                match=corpora.Corpus.matchReferenceInIndex(ref)
                if match:
                    ref["resolved_link"]=match["guid"]
                    res.append({"cit":cit,"match":match})
                    ...
                    sents_to_fix.append(sent)
            else:
                print("no match by id! %s -> %s" % (cit["id"],cit["link"]))
        else:
            print ("cit link == 0! %s " % cit["id"])

    for sent in sents_to_fix:
        countMultiCitations(sent, doc) # deal with many citations within characters of each other: make them know they are a cluster TODO cluster them
    ...
    """
        ...
        Args:
            detected_style: one of the known styles (e.g. "APA" or "AFI") for
                ...
                it calls detectCitationStyle() to find out
        Returns:
            tuple: (new sentence text, list of dicts with structured info)
    """
    if not detected_style:
        detected_style=detectCitationStyle(text)
...
def annotateCitationsAPA(text):
    ...
    def repFunc(match):
        ...
        cit_dict={
            ...
            "year":match.group(7) if match.group(5) else match.group(6)
        }
