...
# ------------------------------------------------------------------------------
    ...
    def extractInfoFromPatentText(self, text, newRef):
        """
            ...
            Args:
                text: the text content of the reference
                newRef: the already added reference dictionary
        """
    ...
    def loadJATSMetadataFromPaper(self, newDocument, soup):
        ...
        for a in meta.findChildren("contrib", {"contrib-type":"author"},recursive=False):
    ...
    def loadJATSSection(self, sec, newDocument, parent):
        ...
        if contents:
            for element in contents:
                if element.name=="sec":
                    self.loadJATSSection(sec2,newDocument,newSection_id)
                elif element.name=="p":
                    newPar_id=self.loadJATSParagraph(p, newDocument, newSection_id)
...
##        par_text=re.sub("</?p>","",p.__repr__(),flags=re.IGNORECASE)
        ...
        for s in sentences:
            ...
            citations_found=[]
            for r in refs:
                citations_found.extend(self.loadJATSCitation(r, newSent_id, newDocument, parent))
##                re.sub(r"<xref\s+.*?</xref>",)

            # adapt this code to replace <xref> tags
...
##
##                text=re.sub(r"<cit\sid="+str(cit_id)+"\s*?/>",sub,text, flags=re.DOTALL|re.IGNORECASE)
...
def generateSideBySide(doc_list):
    ...
    ## c:\\users\\dd\\documents\\dropbox\\phd\\code\\xmlformats
##    output_dir="..\\xmlformats\\conversion_visualization\\"
    output_dir="g:\\nlp\\phd\\pmc\\conversion_visualization\\"
...
def main():
    ...
    debugging_files=[
    "articles.I-N\J_Contemp_Brachytherapy\J_Contemp_Brachytherapy_2012_Sep_29_4(3)_176-181.nxml",
    "articles.O-Z\PLoS_ONE\PLoS_One_2013_Dec_20_8(12)_e85076.nxml",
    "articles.C-H\Gastroenterol_Rep_(Oxf)\Gastroenterol_Rep_(Oxf)_2013_Sep_17_1(2)_149-152.nxml",
    ]

    debugging_files=corpora.Corpus.selectRandomInputFiles(500,"*.nxml")
