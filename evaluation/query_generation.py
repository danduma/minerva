...
##            assert this_query["query_text"] != ""
    ...
    def precomputeQueries(self,exp):
        """
            Precompute all queries for all annotated citation contexts (Athar cp.Corpus)

            Args:
                exp: experiment dict with all options
        """
        self.exp=exp
...
class ExplainQueryGenerator(QueryGenerator):
    """
    """

    def precomputeQueries(self, exp):
        """
            Run the whole Citation Resolution pipeline, store the results for each
            citation to then just test different weight values for overall or
            per-citation testing
