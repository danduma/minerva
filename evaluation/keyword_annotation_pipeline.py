...
from db.result_store import ElasticResultStorer, ResultDiskReader, ResultIncrementalReader, OfflineResultReader
...
from ml.keyword_support import saveOfflineKWSelectionTraceToCSV, saveOfflineFeaturesToJson
...
class KeywordAnnotationPipeline(PrecomputedPipeline):
    """
    ...
    def __init__(self, retrieval_class=BaseRetrieval, use_celery=False):
        super(KeywordAnnotationPipeline, self).__init__(retrieval_class=retrieval_class, use_celery=use_celery)
        self.current_guid = ""
        self.current_doc = None
        self.save_terms = True
        self.annotator = None
        self.writer_name = "kw_data"

    def initializePipeline(self):

        super(KeywordAnnotationPipeline, self).initializePipeline()
    ...
    def annotateDocuments(self):
        ...
        if self.options.get("run_feature_annotation", False):
            self.annotator = DocumentFeaturesAnnotator(self.exp["features_index_name"],
                                                       field_name=self.exp["features_field_name"],
                                                       experiment=self.exp)
            all_guids = set()
            ...
            for guid in pbar:
                ...
                self.annotator.annotate_scidoc(doc)
    ...
    def addResult(self, file_guid, precomputed_query, doc_method, retrieved_results):
        ...
        if "precomputed_feature_data" not in self.current_doc.data:
            print(self.current_doc.metadata["guid"], "not annotated. Annotating...")
            if not self.annotator:
                self.annotator = DocumentFeaturesAnnotator(self.exp["features_index_name"],
                                                           field_name=self.exp["features_field_name"],
                                                           experiment=self.exp)

            self.annotator.annotate_scidoc(self.current_doc)
            cp.Corpus.saveSciDoc(self.current_doc)
    ...
    def cacheResultsLocally(self, exp=None):
        """
            Downloads all results from elastic to json in the experiment directory
        """
        # self.writers["ALL"].getResultList(0) # update list of items
        # self.writers["ALL"].refreshIndex()
        if exp:
            self.exp = exp
        storer = ElasticResultStorer(self.exp["name"], self.writer_name, endpoint=cp.Corpus.endpoint)
        storer.refreshIndex()
        reader = ResultDiskReader(storer, os.path.join(self.exp["exp_dir"], "cache"))
        reader.emptyCache()
    ...
    def exportAnnotationResults(self):
        ...
        feature_data_filename = self.exp.get("feature_data_filename", "feature_data.json.gz")

        saveOfflineFeaturesToJson(self.writer_name,
                                  os.path.join(self.exp["exp_dir"], "cache"),
                                  os.path.join(self.exp["exp_dir"], feature_data_filename))
