...
def shouldIgnoreCitation(source_metadata, target_metadata, filter_options):
    ...
    if filter_options.get("limit_to_same_collection", False):
        if source_metadata.get("collection_id") and target_metadata.get("collection_id") and source_metadata["collection_id"] != target_metadata.get("collection_id"):
            return True
...
class BaseCorpus(object):
    ...
    def selectDocResolvableCitations(self, doc, filter_options={}):
        ...
        for ref in doc["references"]:
            match = self.matcher.matchReference(ref, doc)
            doc_meta=self.getMetadataByGUID(doc.metadata["guid"])
            if match:
                if not shouldIgnoreCitation(doc_meta, match, filter_options):
                    ref["guid"] = match["guid"]
