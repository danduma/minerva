...
from proc.nlp_functions import (stopwords, replaceCitationTokensForParsing,
...
##                tf=df_scores.get(text,{}).get("term_freq",1)
                ...
                features={"text":text,
                          "pos":token.pos_, # FIXME: Why almost all POS == 0?
                          "dep":token.dep_, # FIXME: DEPENDENCIES ARE MISSING! How to get them?
                          "ent_type":token.ent_iob, # this is useless for scientific papers. Need a corpus-specific NER
                          "lemma":token.lemma_ or text, # TODO: check other option, not lemma
