...
ILC_AZ_LIST = ["ilc_AZ_" + zone for zone in AZ_ZONES_LIST]
ILC_CORESC_LIST = ["ilc_CSC_" + zone for zone in CORESC_LIST]
...
rx_word_boundaries = re.compile('\w+', re.IGNORECASE)
...
def replaceCitationsWithPlaceholders(text):
    ...
    return re.sub(r"\_\_cit(d+)", CITATION_PLACEHOLDER, text)


def replaceXMLCitationsWithUnderscoreCitations(text):
    """
        Substitutes all <CIT ID=cit6/> elements in the sentence with __cit6
    """
    return re.sub(r"<CIT ID=(.+?)\s?/>", r"__\1", text)


def getCitationNumberFromTokenText(text):
    """
        Returns the citation number from a single-token citation
    """
    match = re.search(r"(?:__)?cit(\d+)", text, flags=re.IGNORECASE)
    if not match:
        return None
...
def selectSentencesToAdd(docfrom, cit, param):
    ...
    if cit["parent_s"] not in docfrom.element_by_id:
        match = re.search("s(\d+)", cit["parent_s"])
        if not match:
            print("Parent sentence of citation is in unexpected format", cit)
            return []
        ...
        while not found:
            new_id = "s" + str(s_id)
            if new_id not in docfrom.element_by_id:
                s_id -= 1
                continue

            if cit["id"] in docfrom.element_by_id[new_id].get("citations", []):
                cit["parent_s"] = new_id
                found = True
                break
        ...
        if not found:
            print("Couldn't find citation ", cit)
            return []
...
def leaveOneCitationToken(tokens, cit_number):
    ...
    for index, token in enumerate(tokens):
        ...
        if previous_was_cit and token_text in [",", ";"]:
            continue

        previous_was_cit = False

        if token_text.startswith(CIT_MARKER):
            if token_text == CIT_MARKER + str(cit_number):
                res.append(CIT_MARKER)
            # else don't append the token
            previous_was_cit = True
        else:
            res.append(token_text)

    sentence = " ".join(res)
    sentence = re.sub(r"\(\s*\)", " ", sentence)
    sentence = re.sub(r"\(\s*" + CIT_MARKER + r"\s*\)", CIT_MARKER, sentence)
    sentence = re.sub(r"\s+\.", ".", sentence)
    sentence = re.sub(r"\s+,", ",", sentence)
...
def leaveOneCitationToken(tokens, cit_number, trace_other_citations=False):
    """
    Removes all __cit tokens in the sentence that aren't the one we care about, including commas in between them
    ...
    for index, token in enumerate(tokens):
        ...
        if token_text.startswith(CIT_MARKER):
            if token_text == CIT_MARKER + str(cit_number):
                res.append(CIT_MARKER)
            elif trace_other_citations and not previous_was_cit:
                res.append(CIT_TRACE)
            # else don't append the token
            previous_was_cit = True
        else:
            res.append(token_text)
    ...
    sentence = re.sub(r"\(\s*\)", " ", sentence)
    sentence = re.sub(r"\(\s*" + CIT_MARKER + r"\s*\)", CIT_MARKER, sentence)
    sentence = re.sub(r"\(\s*" + CIT_TRACE + r"\s*\)", CIT_TRACE, sentence)
    sentence = re.sub(CIT_TRACE + r"\s*" + CIT_MARKER, CIT_MARKER, sentence)
    sentence = re.sub(r"\s+\.", ".", sentence)
    sentence = re.sub(r"\s+,", ",", sentence)
...
def leaveOneAuthorToken(tokens):
    """
    Removes all __cit tokens in the sentence that aren't the one we care about, including commas in between them
    ...
    sentence = " ".join(res)
    sentence = re.sub(r"\(\s*\)", " ", sentence)
    sentence = re.sub(r"\(\s*" + AUTHOR_MARKER + r"\s*\)", " %s " % AUTHOR_MARKER, sentence)
    sentence = re.sub(r"\s*" + AUTHOR_MARKER + r"\s*(?:and)?\s*" + AUTHOR_MARKER, " %s " % AUTHOR_MARKER, sentence)
    sentence = re.sub(r"\s*" + AUTHOR_MARKER + r"\s*(?:and)?\s*" + AUTHOR_MARKER, " %s " % AUTHOR_MARKER, sentence)
    sentence = re.sub(r"\s+\.", ".", sentence)
    sentence = re.sub(r"\s+,", ",", sentence)
    sentence = re.sub(r"\s+", " ", sentence)
