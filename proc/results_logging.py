...
def measureScores(result_guids, match_guids, result_dict, prepend_text="", BIG_VALUE=200):
    """
        ...
        {'mrr_score': 1.0, 'precision_score': 1.0, 'ndcg_score': 1.0, 'rank': 1, 'map_score': 1.0, 'per_guid_rank': {'a': 1}}
        >>> print(measureScores(["a", "b", "c", "d", "e"], ["b", "d"], scores))
        {'mrr_score': 0.625, 'precision_score': 0.5, 'ndcg_score': 0.7153382790366966, 'rank': 2, 'map_score': 0.6666666666666666, 'per_guid_rank': {'b': 1, 'd': 4}}
    """
    ...
    for index, guid in enumerate(result_guids):
        ...
        if guid in match_guids and guid not in found_guids:
            ...
            per_guid_rank[guid] = results[-1]["rank"]
    ...
    for res in results:
        if res["rank"] == -1:
            ranks.append(BIG_VALUE)
        else:
            ranks.append(res["rank"])

    rank = int(sum(ranks) / len(match_guids))
    ...
    result_dict["per_guid_rank"] = per_guid_rank
    return result_dict
...
class ResultsLogger(ProgressIndicator):
    ...
    def showFinalSummary(self, overlap_in_methods=None):
        ...
        overall_ndcg = defaultdict(lambda: 0)
        total_points = defaultdict(lambda: 0)

        for qmethod in self.scores["precision"]:
            for method in self.scores["precision"][qmethod]:
                overall_precision[method] += self.scores["precision"][qmethod][method] / float(self.numitems)
                overall_mrr[method] += self.scores["mrr"][qmethod][method] / float(self.numitems)
                overall_ndcg[method] += self.scores["ndcg"][qmethod][method] / float(self.numitems)
        ...
        for method in overall_precision:
            ...
            print("NDCG for", method, "%02.4f" % (overall_ndcg[method] / float(total_points[method])))
    ...
    def measureScoreAndLog(self, retrieved_docs, citation_multi, result_dict):
        ...
        result_dict["query_id"] = self.query_counter
...
def main():
    ...
    measureScores(["a", "b", "c", "d", "e"], ["a", "b"], scores)
    print(scores)
    pass
