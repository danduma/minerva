...
FIELDS_TO_IGNORE = ["left_start", "right_end", "params", "guid_from", "year_from",
                    "authors_from", "authors_to", "guid_to", "year_to", "_metadata", "metadata"]
...
def getDictOfLuceneIndeces(prebuild_indices):
    """
        ...
        FIXME: For some reason this function is different from getDictOfTestingMethods().
...
def joinTogetherContextExcluding(context_list, exclude_files=[], exclude_authors=[], filter_options={}):
    ...
    for cont in context_list:
        cont = makeSureMetadataKeyExists(cont)
    ...
    if "_full_ilc" not in res:
        res["_full_ilc"] = res["inlink_context"]
...
def generateDocBOWInlinkContext(doc_target, parameters, doctext=None, filter_options={}, force_rebuild=False):
    ...
    window_parameters = [param for param in parameters if not isinstance(param, six.string_types)]
    sentence_parameters = [param for param in parameters if isinstance(param, six.string_types)]

    # print("Window parameters", window_parameters)
    # print("Sentence parameters", sentence_parameters)
...
def makeSureMetadataKeyExists(data):
    """
    For old-style BOWs where the _metadata key is missing

    :return:
    """
    assert isinstance(data, dict)

    METADATA_KEYS = ["guid_from", "guid_to", "guid", "year_from"]
...
def mashupBOWinlinkMethods(doc_incoming_guid, exclude_files, index_max_year, method_params, full_corpus=False,
    ...
    for passage in bow_method1:
        ...
        passage["_all_text"] = passage.get("_full_text","") + " " + passage.get("_full_ilc","")
