...
from proc.keyphrase_annotation import getMatchingKPsInContext
from .nlp_functions import (tokenizeText, tokenizeTextAndRemoveStopwords, stopwords,
                            ...
                            replaceCitationsWithPlaceholders, leaveOneCitationToken, leaveOneAuthorToken, AUTHOR_MARKER,
                            CIT_MARKER)
...
class WindowQueryExtractor(BaseQueryExtractor):
    ...
    def tokenizeContext(self, params):
        ...
        try:
            max_left = ((params["wleft"] + 15) * ESTIMATED_AVERAGE_WORD_LENGTH)
            left_start = max(0, params["match_start"] - max_left)
        except:
            print(params)
    ...
    def extractMulti(self, params):
        ...
        for param in params["parameters"]:
            if not isinstance(param, tuple) or isinstance(param, list) or isinstance(param, string_types):
                param = (param, param)
        ...
        for parameter in params["parameters"]:
            ...
            if not params.get("extract_only_plain_text", False):
                ...
                if params.get("annotate_keyphrases", True):
                    kps = getMatchingKPsInContext(extracted_query["text"],
                                                  params["docfrom"].data.get("annotated_keyphrases", []))
                    extracted_query["keyphrases"] = kps
...
class SentenceQueryExtractor(BaseQueryExtractor):
    ...
    # sent=sent.replace(AUTHOR_MARKER, " ")
    # sent=sent.replace(AUTHOR_MARKER, " ")
    # return formatSentenceForIndexing(sent)

    def formatTextForIndexing(self, text):
        text = re.sub("__cit\d+", " ", text)
        text = text.replace(AUTHOR_MARKER, " ")
        text = text.replace(CIT_MARKER, " ")
    ...
    def extract(self, params):
        ...
        for sent_id in to_add:
            ...
            text = params["docfrom"].formatTextForExtraction(sent["text"])
            text = self.formatTextForIndexing(text)
            ...
            extracted_query["vis_text"] = extracted_query.get("vis_text", "") + sent["text"] + " "
        ...
        extracted_query["vis_text"] = params["docfrom"].formatTextForExtraction(extracted_query["vis_text"])
        # print(extracted_query["vis_text"])
        cit_number = params["cit"]["id"].replace("cit", "")
        # print(params["cit"])
        extracted_query["vis_text"] = leaveOneCitationToken(tokenizeText(extracted_query["vis_text"]), cit_number,
                                                            trace_other_citations=True)
        extracted_query["vis_text"] = leaveOneAuthorToken(tokenizeText(extracted_query["vis_text"]))
        # print(extracted_query["vis_text"])
        return extracted_query
