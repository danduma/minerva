# generates the data to visualize overlaps of terms between citing
...
from proc.general_utils import writeFileText, getRootDir
from proc.nlp_functions import tokenizeText, getDictOfTokenCounts
from scidoc.reference_formatting import formatCitation
import proc.nlp_functions as nlp_functions
...
class VisGenerator:
    """
        Class wrapper over functions that generate the visualization data
    """

    def __init__(self):
        self.term_info = {}
        ##          self.output_dir=cp.Corpus.dir_output
        self.output_dir = cp.Corpus.paths["output"]

    def getDocumentTokens(self, doc):
        """
            Returns a dict of all terms in the document
        """
        full_text = doc.formatTextForExtraction(doc.getFullDocumentText(True, False))
    ...
    def citationFormatting(self, formatted_citation, reference):
        """
            ...
            info from reference
        """
        extra_class = "in-collection_cit " if reference.get("in_collection") else ""
    ...
    def generateVisualizationOneFile(self, guid):
        ...
        for ref in doc["references"]:
            ...
            if match:
                ...
                renderer = SciDocRenderer(doc2)
                ref_html = renderer.prettyPrintDocumentHTML(
                    True,
                    True,
                    False,
